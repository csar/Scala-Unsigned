...
class UnsignedSuite extends FunSuite {
  ...
  test("divide longs") {
    ...
    for (c <- 0 to 1000000 * multiplier) {
      ...
      val u1: ULong = i
      val u2: ULong = j
      assert((u1 / u2).toBigInt === u1.toBigInt / u2.toBigInt,
             s"$u1 / $u2 $i $j")
      assert((u1 % u2).toBigInt === u1.toBigInt % u2.toBigInt,
             s"$u1 % $u2 $i $j")
    }
  ...
  test("multiply ints") {
    ...
    for (c <- 0 to 1000000 * multiplier) {
      ...
      val u1: UInt = i
      val u2: UInt = j
      assert((u1 * u2).toLong === ((u1.toLong * u2.toLong) & ((1l << 32)) - 1),
             s"$u1 * $u2 $i $j")
    }
  }
  test("multiply longs") {
    ...
    for (c <- 0 to 1000000 * multiplier) {
      ...
      val u2: ULong = j
      assert(
        (u1 * u2).toBigInt === ((u1.toBigInt * u2.toBigInt) & ((BigInt(1) << 64)) - 1),
        s"$u1 * $u2 $i $j"
      )
    }
  }
  test("multiply shorts") {
    for (c <- 0 to 1000000 * multiplier) {
      ...
      val u1: UShort = i.toShort
      val u2: UShort = j.toShort
      assert((u1 * u2).toInt === ((u1.toInt * u2.toInt) & ((1 << 16)) - 1),
             s"$u1 * $u2 $i $j")
    }
  }
  test("multiply bytes") {
