...
package object unsigned {
  ...
  trait UShortIsIntegral extends Integral[UShort] {
    ...
    def times(x: UShort, y: UShort): UShort =
      ((x.short * y.short) & 0xffff).toShort
  ...
  trait ULongIsIntegral extends Integral[ULong] {
    ...
    def toDouble(x: ULong): Double =
      if (x.long < 0) java.lang.Long.MAX_VALUE * 2.0 + x.long
      else x.long.toDouble
    def toFloat(x: ULong): Float =
      if (x.long < 0) java.lang.Long.MAX_VALUE * 2.0f + x.long
      else x.long.toFloat
